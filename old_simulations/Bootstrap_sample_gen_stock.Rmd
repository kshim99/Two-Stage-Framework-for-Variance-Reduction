---
title: "Bootstrap_sample_gen_stock"
author: "Kyu Min Shim"
date: "2023-11-27"
output: pdf_document
---
...
stock = read.csv(data_dir)
```
## select only response and date
```{r}
# pick an outcome column
data = rename(stock, ds = Date, y = Close)
data$ds = ymd(data$ds)
data = data[,c("ds","y")]
```
```{r}
...
plot(train$ds, train$y, xlim = c(min(train$ds), max(test$ds)), ylim = c(min(train$y), max(train$y)), type = 'l', col = 'black', ylab = 'Stock Price', xlab = 'Time')
title('Yahoo Stock Price')
...
# (p,q): (0,1), (0,2), (0,8), (0,11), (1,1), (1,2), (1,8), (1,11)
# (P,Q): (0,0)
```
```{R}
# define set of params to try for SARIMA model
params1 = rbind(c(0,1,1),c(0,1,2),c(0,1,8),c(0,1,11),c(1,1,1),c(1,1,2),c(1,1,8),c(1,1,11))
params2 = rbind(c(0,0,0))
...
opt_p2 = params2[0,]
m = 0
model1 = arima(train_y, order = opt_p1)
...
for (i in 1:num_sample) {
  ...
  while (!(IdentInvertQ(phi = coef_sample[1], theta = coef_sample[2:12]))) {
    coef_sample = mvrnorm(1, model1$coef, model1$var.coef)
  }
  mdl = arima(train_y, order=opt_p1, fixed = coef_sample)
...
for (i in 1:sim_num) {
  ...
  treat = test[-control_idx,]
  control_y = control$y
  treat_y = treat$y + rnorm(nrow(treat), te, 20)
...
summary(sim_res$error1/floor(nrow(test)/2) + sim_res$error0/floor(nrow(test)/2)
        + model1$sigma2/floor(nrow(test)/2) + model1$sigma2/floor(nrow(test)/2))
```
when varying predictions, TE variance is 709, when not varying predictions, TE variance is 697. average variance in prediction is 27.57. varying predictions does add more randomness to TE variance, but not in the most explainable way? 
