...
p1 = hist(samples[[1]], col = cols[1], xlim = x_lims, prob = TRUE, breaks = seq(x_lims[1], x_lims[2], bin_width))
...
pc = hist(combined_sample, col = cols[4], xlim = x_lims, prob = TRUE, breaks = seq(x_lims[1], x_lims[2], bin_width))
```
...
est_params = data.frame(matrix(nrow = 3, ncol = num_cls))
rownames(est_params) = c("Delta","Alpha","Theta")
colnames(est_params) = seq(1,num_cls)
